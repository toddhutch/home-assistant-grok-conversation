...
from . import GrokConfigEntry  # Reason: Updated from OpenAIConfigEntry to match __init__.py type hint
from .const import (
    CONF_CHAT_MODEL,
    CONF_MAX_TOKENS,
    CONF_PROMPT,
...
async def async_setup_entry(
    hass: HomeAssistant,
    config_entry: GrokConfigEntry,  # Reason: Changed to GrokConfigEntry for consistency
    async_add_entities: AddConfigEntryEntitiesCallback,
) -> None:
    """Set up conversation entities."""
    # Reason: Renamed to GrokConversationEntity to reflect the new integration
    agent = GrokConversationEntity(config_entry)
...
def _format_tool(
    ...
    # Reason: No changes needed; Grok’s API supports the same tool format as OpenAI
...
def _convert_content_to_param(
    ...
    # Reason: Unchanged; message format is compatible with Grok’s API
...
async def _transform_stream(
    ...
    """Transform a Grok delta stream into HA format."""
    # Reason: Logic unchanged as it’s generic; only docstring updated for clarity
...
class GrokConversationEntity(
    ...
    # Reason: Renamed from OpenAIConversationEntity to indicate Grok integration
    ...
    def __init__(self, entry: GrokConfigEntry) -> None:
        ...
        self._attr_device_info = dr.DeviceInfo(
            ...
            name=entry.title,
            manufacturer="xAI",  # Reason: Changed from "OpenAI" to "xAI", Grok’s creator
            model="Grok",       # Reason: Changed from "ChatGPT" to "Grok" for accuracy
    ...
    async def _async_handle_message(
        ...
        model = options.get(CONF_CHAT_MODEL, RECOMMENDED_CHAT_MODEL)  # Reason: Uses "grok-2" from const.py
        ...
        for _iteration in range(MAX_TOOL_ITERATIONS):
            ...
            # Reason: Commented out reasoning_effort; it’s OpenAI-specific and not supported by Grok
            ...
            try:
                ...
                LOGGER.error("Error talking to Grok: %s", err)  # Reason: Updated to "Grok" for clarity
